...
#preparecommand = '/opt/pdflabs/pdftk/bin/pdftk %s burst output %s ' %(dirpath + pdf, dirpath + namePage)
                preparecommand = '/opt/pdflabs/pdftk/bin/pdftk {} burst output {}'.format(dirpath + pdf, dirpath + namePage)
...
#preparecommand = 'inkscape %s --export-plain-svg=%s' %(pdfpath + page, svgpath + svgfile)
                    preparecommand = '/Applications/Inkscape.app/Contents/MacOS/inkscape --batch-process --export-plain-svg -o {} {}'.format(svgpath+svgfile, pdfpath + page)
                    system(preparecommand)
